...
import { languageFromExt, sha256Hex, chunkFileContent } from "@mp/search/chunker";
 ...
 * Stored at `<workspacePath>/.moneypenny/workspace.sqlite`.
 */
export function createWorkspaceDB(
  workspacePath: string,
  opts?: { modelPath?: string },
): WorkspaceDB {
  const mpDir = join(workspacePath, ".moneypenny");
  if (!existsSync(mpDir)) {
