...
Extracting a variable may be the first step towards performing [Extract Method](/extract-method) if you see that the extracted expression is used in other places in your code.
...
- When refactoring conditional expressions, remember that the compiler will most likely optimize it to minimize the amount of calculations needed to establish the resulting value. Say you have the following expression `if (a() || b()) ...`. The program won't call the method `b` if the method `a` returns `true` because the resulting value will still be `true`, no matter what value returns `b`.
