...
The most obvious reason for using this refactoring technique is related to [Replace Type Code with Subclasses](https://refactoring.guru/replace-type-code-with-subclasses).
...
Factory methods can be used in other situations as well, when constructors aren't up to the task. They can be important when attempting to [Change Value to Reference](https://refactoring.guru/change-value-to-reference). They can also be used to set various creation modes that go beyond the number and types of parameters.
