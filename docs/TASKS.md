...
- [ ] `SqliteAiProvider` — replace stub with real implementation (blocked on sqlite-ai extension compilation)
...
- [ ] Wire `LocalEmbeddingProvider` to sqlite-ai `ai_embed()` (blocked on extension compilation)
...
- [ ] Wire vector similarity via `sqlite-vector` (blocked on extension compilation; currently using text/Jaccard as proxy)

## M6 — Context Assembly & Token Budget ✅

- [x] Token budget allocator with reserved/flexible split
- [x] Full assembly pipeline (system prompt → policies → fact pointers → expanded facts → scratch → log → knowledge → current message)
- [x] Dynamic rebalancing for empty scratch, new session, deep session
- [ ] Rolling conversation summaries (incremental summarization every N turns)
...
- [ ] MCP tool discovery (blocked on sqlite-mcp extension)
- [ ] `sqlite-js` tool persistence (blocked on sqlite-js extension)
...
## M13 — Gateway & Multi-Agent ✅ (core runtime)
...
- [ ] Sync integration via `sqlite-sync` (blocked on extension)
- [ ] Inter-worker message routing from gateway (protocol exists, routing logic TBD)

## M14–M17 — Adapters & UI ❌ (not started)
...
**Phase 2–4 — Compilation & linking: BLOCKED**
- [ ] Fix sqlite-sync SQLite header version mismatch (`sqlite3_get_clientdata` requires 3.44+)
- [ ] Complete C/C++ compilation for all 7 extensions
- [ ] CMake builds for llama.cpp + whisper.cpp (sqlite-ai)
- [ ] Runtime initialization and smoke tests
- [ ] Wire `sqlite-vector` embeddings into search (unblocks M5)
- [ ] Wire `sqlite-ai` into `SqliteAiProvider` (unblocks M3)
...
**Next — unblock extensions:**
1. Fix extension compilation (cross-cutting Phase 2–4)
2. Vector similarity in search (M5)
3. Rolling conversation summaries (M6)

**Later:**
4. Inter-worker message routing in gateway
5. Channel adapters (M14–M17)
6. Policy DSL (M7 Layer 3)
7. Web UI, WASM, skill marketplace
