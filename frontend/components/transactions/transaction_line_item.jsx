...
export default function transaction_line_item({ transaction }) {
  ...
  return (
    <tr key={id} className="table-row" value={transaction}>
      ...
      <td className="table-row-data transaction-category">{transaction_category}<img onClick={(e) => modalOpener('edit', TransactionFormContainer, transaction)} className="transaction-gear" src={window.pencil} alt="gear" /></td>
