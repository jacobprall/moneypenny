...
use common::{init_project, CONFIG_NAME};
...
fn init_creates_agent_db_and_metadata_db() {
    ...
    assert!(data_dir.join("main.db").exists(), "main agent DB should exist");
    assert!(data_dir.join("metadata.db").exists(), "metadata DB should exist");
}
...
fn init_creates_models_directory() {
    ...
    assert!(models.is_dir(), "models dir should exist: {}", models.display());
}
