...
fn health_succeeds_after_init() {
    ...
    assert!(
        out.status.success(),
        "mp health should succeed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("Moneypenny") || stdout.contains("Gateway") || stdout.contains("Agent"),
        "health output: {}",
        stdout
    );
}
...
fn agent_status_succeeds_after_init() {
    ...
    assert!(
        out.status.success(),
        "mp agent status should succeed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
